...
class AdvString(str):
    ...
    def get_bytecode(self, encoding="utf-8"):
        return bytes(self.encode(encoding=encoding))
    ...
    def dill(self):
        import dill
        return AdvBytes(dill.dumps(self))
...
class AdvFloat(float):
    ...
    @staticmethod
    def get_bytecode(as_="d"):
        """
        Get the bytecode of the float.

        :param as_: Use 'd' for c-double, or 'f   for c-float. Uses data formed by the C language.
        :returns: The bytecode of the float, generated by the struct.pack() function.
        :raises ValueError: If argument 'as_' isn't either 'd' or 'f'.
        :raises TypeError: If argument 'as_' is not a string.
        """

        if not isinstance(as_, str):
            raise TypeError(f"Invalid type for argument 'as_': {type(as_).__name__}")

        if as_ not in ['d', 'f']:
            raise ValueError(f"Invalid c-type for struct: {repr(as_)}, see __doc__ for details.")

        import struct
        return struct.pack(as_)
...
class AdvBytes(bytes):
    ...
    def compile_bool(self):
        import struct
        integer = struct.unpack("i", bytes(self))[0]
        if 0 <= integer <= 1:
            return bool(integer)
        else:
            raise OverflowError("Byte data too large for bool value.")

    def compile_string(self, encoding="utf-8"):
        self.decode(encoding=encoding)
...
if __name__ == '__main__':
    ...
    this_is_a_var = pickle.dumps({(345, 35): lambda: str()})
